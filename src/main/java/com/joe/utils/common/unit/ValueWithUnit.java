...
public abstract class ValueWithUnit<UNIT extends UnitDefinition>
    ...
    /**
     * 增加指定值，最终单位以当前值的单位一致
     * @param unitValue 要增加的值
     * @param <T> 值类型，必须与当前类型一致
     */
    public <T extends ValueWithUnit<UNIT>> void add(T unitValue) {
        Assert.notNull(unitValue);
        // 算出进制，最大支持小数点后50位，超过该值不支持
        BigDecimal radix = new BigDecimal(unitValue.unit.getRadix())
            .divide(new BigDecimal(this.unit.getRadix()), scale, roundMode);
    ...
    public void changeUnit(UNIT unit) {
        ...
        BigDecimal radix = new BigDecimal(this.unit.getRadix())
            .divide(new BigDecimal(unit.getRadix()), scale, roundMode);
    ...
    public int compareTo(ValueWithUnit<UNIT> o) {
        ...
        BigDecimal thisValue = this.value
            .multiply(new BigDecimal(this.unit.getRadix() / unit.getRadix()));
