...
public class ThreadUtil {
    ...
    public static void join(Thread thread) {
        ExceptionWraper.run(thread::join, e -> new WrapedInterruptedException((InterruptedException)e));
    }
     ...
     *            线程池类型
     * @param format
     *            线程名格式，格式为：format-%d，其中%d将被替换为从0开始的数字序列，不能为null
     * @return 指定类型的线程池
     */
     ...
     * 
     * @param format
     *            线程名字模板，格式为：format-%d，其中%d将被替换为从0开始的数字序列，不能为null
