...
public class ReflectUtil {
    ...
    public static <R> R invoke(Object obj, String methodName, Class<?>[] parameterTypes,
                               Object... args) {
        Assert.isFalse(
            CollectionUtil.safeIsEmpty(parameterTypes) ^ CollectionUtil.safeIsEmpty(args),
            "方法参数类型列表必须和方法参数列表一致");
     ...
     * 执行方法
     * @param method 要调用的方法
     * @param target 方法所在Class的实例，对于静态方法要传入null或者Class对象
    ...
    public static List<Method> getAllAnnotationPresentMethod(Class<?> type,
        ...
        return methods.stream().filter(method -> method.isAnnotationPresent(annotation))
    ...
    /**
     * 设置指定对象指定对象名的字段值
     * @param obj 对象，如果要设置的字段是静态字段那么请传入静态字段所在的Class对象
     * @param fieldName 字段名
