...
public interface InvokeDistributeFactory {
     ...
     * 
     * @param clazz
     *            给定Class对象，必须是public修饰的，并且有一个公共的无参构造器，并且不包含带有给定Class类型的参数的构造
     ...
     *            指定对象
     * @param className
     *            生成的class将会使用该className，可以为null，为null时应由实现类生成
     * @param classLoader
     *            用于加载生成的动态类的classLoader，可以为null，为null时应由实现类自主决策
     * @return 对target生成的InvokeDistribute代理
     */
    @SuppressWarnings("unchecked")
    default InvokeDistribute build(Object target, String className, DynamicClassLoader classLoader) {
        Class<InvokeDistribute> clazz = build(target.getClass(), className, classLoader);
        return ClassUtils.getInstance(clazz, CollectionUtil.array(target.getClass()), CollectionUtil.array(target));
    }
}
