...
public class SignatureUtilImpl implements SignatureUtil {
    ...
    private SignatureUtilImpl(String privateKey, String publicKey, Algorithms algorithms) {
        ...
        CACHE.computeIfAbsent(this.id, id -> {
            ObjectPoolImpl<SignatureHolder> pool =
                new ObjectPoolImpl<>(() -> buildSignatureHolder(privateKey, publicKey, algorithms));
            // 快速验证
            pool.get().close();
            return pool;
        });
    ...
    private static SignatureHolder buildSignatureHolder(String privateKey, String publicKey, Algorithms algorithms) {
        ...
        try {
            log.debug("构建公钥以及验签器");
            RSAPublicKey pubKey =
                (RSAPublicKey)KeyTools.getPublicKeyFromX509("RSA", new ByteArrayInputStream(publicKey.getBytes()));
            Signature verify = Signature.getInstance(algorithms.toString());
            verify.initVerify(pubKey);

            log.debug("构建私钥以及签名器");
            RSAPrivateKey priKey =
                (RSAPrivateKey)KeyTools.getPrivateKeyFromPKCS8("RSA", new ByteArrayInputStream(privateKey.getBytes()));
